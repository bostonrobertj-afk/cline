...
- `FR-10g1`: The shared pre-workflow project-selection gate must treat that `fs.readdir(..., { withFileTypes: true })` plus `Dirent` filtering flow as its canonical filesystem-discovery method rather than glob-driven discovery, hidden registries, or workflow-specific discovery tooling.
- `FR-10g2`: The shared pre-workflow project-selection gate must consume the same shared runtime-owned discovery/list-builder seam used by workflow-form selection flows so existing-project selection and in-workflow existing-artifact selection follow one canonical methodology.
...
- `FR-21c`: A Module Build action plan must not be considered complete until every legacy workflow-related surface touched by that workflow module build has been implemented in alignment with its approved migration-matrix disposition.
...
- `FR-39f`: When a workflow form asks the user to select an existing project, folder, file, or artifact, `WorkflowRuntime` must populate that form control from the shared discovery/list-builder seam, and after form submission must persist the user's selected canonical value through the workflow-value persistence seam defined in `FR-35a` through `FR-35h`.
...
- `FR-52a`: The Foundational Build phase must not be considered complete until every legacy workflow-related surface touched by the foundational action plan has been implemented in alignment with its approved migration-matrix disposition.
...
- `FR-54a`: The Cleanup phase must not be considered complete until every remaining legacy workflow-related surface assigned to Cleanup by the migration matrix has been handled in alignment with its approved migration-matrix disposition.
...
| Legacy prompt-context field `activeWorkflowPersonaInstructions` | `src/core/prompts/system-prompt/types.ts` field `activeWorkflowPersonaInstructions` | Delete | Runtime-projected workflow system instructions block under `FR-31`, `FR-36`, and `FR-56` | Legacy standalone workflow-persona prompt-context field. Preserve the capability by carrying workflow persona text only through the runtime-projected workflow system instructions block rather than a dedicated prompt-context field. |
...
| Legacy prompt-context field `activePlaceholderWorkflowStepNumber` | `src/core/prompts/system-prompt/types.ts` field `activePlaceholderWorkflowStepNumber` | Update | Runtime-projected active workflow step identity prompt-context field `activeWorkflowStepNumber` under `FR-56`, with `WorkflowRuntime`-owned next-action, tool-schema, and prompt projection under `FR-15`, `FR-29` through `FR-29e`, and `FR-35` through `FR-37` | Legacy placeholder step-number prompt-context field. Preserve the capability to project the active workflow step identity, but retire this placeholder-specific field name in favor of the runtime-owned `activeWorkflowStepNumber` seam. |
...
| Legacy subagent workflow bootstrap seam in `SubagentRunner.ts` | `src/core/task/tools/subagent/SubagentRunner.ts` | Update | `WorkflowRegistry.getWorkflowSkillMetadata()` plus `WorkflowRegistry.resolveWorkflowByUseSkillName(...)` and child-session activation/prompt projection through `WorkflowRuntime` | Keep `SubagentRunner.ts` as the child execution bootstrap seam, but remove legacy workflow discovery, placeholder/managed activation, placeholder-derived parent/child inheritance, BMAD reminder/prompt assembly, and deterministic-placeholder progression ownership. |
...
| Legacy shared workflow tool-id surface in `shared/tools.ts` | `src/shared/tools.ts` | Update | Foundational shared workflow tool-id surface centered on `set_workflow_values` and the unified workflow document-generation tool | Retire `SET_WORKFLOW_PLACEHOLDERS` and remove workflow-specific document-generation tool ids from the canonical workflow runtime tool surface. |
| Legacy backend workflow tool-contract surface in `backendWorkflowToolContracts.ts` | `src/core/task/tools/backendWorkflowToolContracts.ts` | Update | Canonical backend contracts for `set_workflow_values` and the shared workflow document-generation tool | Replace placeholder persistence contracts and stop treating workflow-specific file/document tools or `continue_brainstorming_session` as foundational canonical workflow-tool surfaces. |
| Legacy response-tool registry workflow entry in `ResponseToolRegistry.ts` | `src/core/task/tools/response/ResponseToolRegistry.ts` | Update | Response-tool registry exposing `set_workflow_values` in the foundational workflow tool surface | Remove `SET_WORKFLOW_PLACEHOLDERS` from the live response-tool registry and align workflow tool exposure to the canonical workflow-value persistence tool. |
...
| Legacy task tool-config workflow seam in `TaskConfig.ts` | `src/core/task/tools/types/TaskConfig.ts` | Update | `TaskConfig` carrying shared `workflowRuntime` | Tool handlers and subagent bootstrap must delegate workflow behavior through `WorkflowRuntime` instead of mutating workflow state directly. |
...
| Legacy workflow-form/start-card proto contract file `task.proto` | `proto/cline/task.proto` | Update | Shared proto contracts with `workflowFormId` plus structured project-selection start-card submission | Replace continue-only and resolver-era workflow request shapes with the approved foundational workflow-form and pre-workflow project-selection contracts. |
| Legacy workflow start-card type surface in `workflow-start-card/types.ts` | `src/core/task/workflow-start-card/types.ts` | Update | Shared project-selection workflow start-card state contract | Keep workflow start cards as a generic specialist capability, but remove markdown-body-only and continue-only assumptions. |
| Legacy workflow start-card registry `WorkflowStartCardRegistry.ts` | `src/core/task/workflow-start-card/WorkflowStartCardRegistry.ts` | Delete | Workflow-module-owned start-card definitions consumed by `WorkflowRuntime` | Static workflow-name-to-start-card-copy registry. Preserve the capability by moving per-workflow start-card definitions into workflow modules and having `WorkflowRuntime` resolve and project the active start-card state; do not keep this registry as a transitional compatibility seam. |
| Legacy workflow start-card payload builder `buildWorkflowStartCardPayload.ts` | `src/core/task/workflow-start-card/buildWorkflowStartCardPayload.ts` | Update | `WorkflowRuntime`-fed project-selection start-card payload builder | Build start-card payloads from runtime discovery output and canonical workflow identity instead of fixed title/body/CTA-only markdown assumptions. |
...
| Legacy workflow-form tool dictionary builder `buildToolDictionary.ts` | `src/core/task/workflow-form/dictionaries/buildToolDictionary.ts` | Update | Workflow-start runtime dictionary aligned to `set_workflow_values` | Replace placeholder-era workflow-start tool-reference copy with the foundational workflow-value persistence tool wording while keeping generic dictionary rendering mechanics unchanged. |
...
| Legacy step-resolution runtime `WorkflowStepResolutionRuntime.ts` | `src/core/task/workflow-step-resolution/WorkflowStepResolutionRuntime.ts` | Update | Shared deterministic-execution infrastructure supplied definitions by `WorkflowRuntime` | Remove the live registry dependency and keep this class only as generic deterministic-execution infrastructure. |
...
| Legacy workflow UI renderer in `ChatRow.tsx` | `webview-ui/src/components/chat/ChatRow.tsx` | Update | Webview rendering aligned to `WorkflowForm` and the expanded workflow start-card payload | Update row rendering to the renamed workflow-form contract and the structured project-selection start-card UI flow. |
| Legacy workflow request-builder hook in `useMessageHandlers.ts` | `webview-ui/src/components/chat/chat-view/hooks/useMessageHandlers.ts` | Update | Workflow-form and start-card request builders aligned to the updated shared proto/message contracts | Replace continue-only start-card submission and resolver-era naming with structured project-selection submission and `workflowFormId`. |
...
| Legacy workflow tool initialization file `tools/init.ts` | `src/core/prompts/system-prompt/tools/init.ts` | Update | Tool initialization exposing `set_workflow_values` as the canonical workflow-value persistence tool | Stop initializing `set_workflow_placeholders`. |
| Legacy workflow tool export surface in `tools/index.ts` | `src/core/prompts/system-prompt/tools/index.ts` | Update | Tool export surface exposing `set_workflow_values` instead of `set_workflow_placeholders` | Remove the legacy placeholder persistence tool export. |
...
| Workflow start-card registry test file `WorkflowStartCardRegistry.test.ts` | `src/core/task/workflow-start-card/__tests__/WorkflowStartCardRegistry.test.ts` | Update | Shared project-selection start-card payload coverage under the foundational workflow-start-card contract | Replace fixed title/body/CTA assertions with coverage for the shared project-selection start-card payload emitted by the foundational runtime/start-card seam. |
...
| Workflow-form registry test file `WorkflowFormRegistry.test.ts` | `src/core/task/workflow-form/__tests__/WorkflowFormRegistry.test.ts` | Update | Registry-facing shared contract coverage only where foundational changes alter what this legacy suite must compile against | Update only the shared contract assertions required by foundational build changes; defer broader behavioral rewrites to workflow-module build and cleanup phases. |
| Workflow step-resolution runtime test file `WorkflowStepResolutionRuntime.test.ts` | `src/core/task/workflow-step-resolution/__tests__/WorkflowStepResolutionRuntime.test.ts` | Update | Shared step-resolution runtime coverage aligned to runtime-owned session and owner contracts | Update session-owner/type assertions if foundational work renames the shared step-resolution contracts. |
...
| Chat row workflow UI test file `ChatRow.test.tsx` | `webview-ui/src/components/chat/ChatRow.test.tsx` | Update | Chat-row rendering coverage aligned to `WorkflowForm` and the structured workflow start-card payload | Update mocked workflow-form payloads to `workflowFormId` terminology, keep generic workflow-form rendering coverage compiling against the renamed shared contract, and add start-card rendering coverage for the structured project-selection payload. |
| Workflow request-builder hook test file `useMessageHandlers.test.tsx` | `webview-ui/src/components/chat/chat-view/hooks/useMessageHandlers.test.tsx` | Update | Submission-builder coverage aligned to `WorkflowForm`/`WorkflowStartCard` and structured start-card submission | Update shared type imports and fixtures to the renamed workflow contracts, and replace the continue-only start-card assertion with structured new/existing project submission coverage. |
| Workflow runtime test file `WorkflowRuntime.test.ts` | `src/core/task/workflow-runtime/__tests__/WorkflowRuntime.test.ts` | Update | Foundational runtime activation, progression, form, completion, teardown, persistence, and resume coverage | Add or expand coverage for the approved foundational `WorkflowRuntime` responsibilities, including project-selection gating, deterministic execution/fallback, completion handling, teardown, persistence, and safe resume. |
