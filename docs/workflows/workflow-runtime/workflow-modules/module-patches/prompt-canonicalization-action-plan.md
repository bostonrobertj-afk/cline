...
- [x] Subtask 1.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/acceptance-audit-review/acceptanceAuditReviewWorkflow.ts`, replace `ACCEPTANCE_AUDIT_REVIEW_STEP_2_PROMPT` with a readable multiline template literal. Preserve the exact rendered AI-facing text and every `{workflow.target_story}`, `{workflow.epics_document}`, `{workflow.architecture_document}`, `{workflow.review_scope_manifest}`, `{workflow.review_commit_hash}`, `{workflow.review_commit_parent}`, and `{workflow.acceptance_audit_output}` token. Do not change `buildStep2PromptSource()`. Do not change `promptTemplates: [ACCEPTANCE_AUDIT_REVIEW_STEP_2_PROMPT]`.
  - Allowed files:
...
## Task 3: Brainstorming Step 3 Prompt Source And Tool Schema

- [x] Subtask 3.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/brainstorming/brainstormingWorkflow.ts`, delete existing constants `STEP_3_SHARED_FACILITATION_PROMPT`, `BRAINSTORMING_STEP_3_SUGGEST_PROMPT_TEMPLATE`, and `BRAINSTORMING_STEP_3_STANDARD_PROMPT_TEMPLATE`; then add these exact constants in their place:
...
Do not add topic refinement, goal refinement, `append_brainstorming_selected_technique`, `set_workflow_values`, `Use the already selected brainstorming technique recorded`, or the text `Offer challenges to to`.
  - Allowed files:
...
- [x] Subtask 3.2. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/brainstorming/brainstormingWorkflow.ts`, replace the body of `buildStep3PromptSource(input: WorkflowPromptBuilderInput): WorkflowStepPromptSource` with this exact section-assembly shape:
...
Do not perform local workflow-value substitution in this function. Do not add a separate full-prompt constant for suggest, choose, or random approaches.
  - Allowed files:
    - `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/brainstorming/brainstormingWorkflow.ts`

- [x] Subtask 3.3. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/brainstorming/brainstormingWorkflow.ts`, replace Step 3 `promptTemplates` with this exact static-section inventory:
...
- [x] Subtask 3.4. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/brainstorming/__tests__/brainstormingWorkflow.test.ts`, inside test `builds Step 3 prompt and tool variants and routes workflow progress decisions`, replace the existing assertions from `expect(suggestPrompt).to.include(OUTPUT_FILE)` through `expect(choosePrompt).not.to.include("{workflow.output_file}")` with these exact assertions and the exact random-approach render block below:
...
```ts
		...
		expect(suggestPrompt.indexOf(`Read \`${OUTPUT_FILE}\``)).to.be.lessThan(
			suggestPrompt.indexOf("apply_patch"),
		)
		expect(suggestPrompt.indexOf("apply_patch")).to.be.lessThan(
			suggestPrompt.indexOf("workflow_progress_request"),
		)
...
In the same test, replace the `approvedStep3ToolNames` array with this exact array:
...
Inside the `for (const step3ToolNames of step3ToolNamesByApproach)` loop, add this exact assertion after `expect(step3ToolNames).not.to.include("set_workflow_values")`:
...
- [x] Subtask 3.5. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/brainstorming/brainstormingToolSchemas.ts`, delete the exported function `buildBrainstormingAppendSelectedTechniqueToolSchema()` in full. In `buildBrainstormingStep3SuggestToolSchemas()`, replace the returned array with this exact array:
...
Do not remove `buildBrainstormingStep3ChooseOrRandomToolSchemas()`. Do not remove the shared backend `append_brainstorming_selected_technique` handler, shared enum value, or backend workflow tool contract outside this file.
  - Allowed files:
...
- [x] Subtask 3.6. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/brainstorming/__tests__/brainstormingToolSchemas.test.ts`, remove `buildBrainstormingAppendSelectedTechniqueToolSchema` from the import list. Replace test `builds schemas for the Phase 3 suggestion-only brainstorming tools` with this exact test:
...
In test `exposes exact Step 3 and Step 4 active-step tool schemas`, replace the `approvedStep3ToolNames` array with this exact array:
...
In test `keeps runtime-owned artifact tools out of model-facing brainstorming schemas`, rename the test to `keeps runtime-owned and non-model-facing tools out of model-facing brainstorming schemas`; then replace the `forbiddenToolNames` array with this exact array:
...
No other imports are added or removed by this subtask.
  - Allowed files:
...
- [x] Subtask 4.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/code-review/codeReviewWorkflow.ts`, update `CODE_REVIEW_STEP_4_UPSTREAM_FAILURE_PROMPT` by deleting only these marker lines from the template literal:
...
- [x] Subtask 4.2. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/code-review/codeReviewWorkflow.ts`, update `CODE_REVIEW_STEP_4_REMEDIATION_STORY_PROMPT` by deleting only these marker lines from the template literal:
...
- [x] Subtask 4.3. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/code-review/codeReviewWorkflow.ts`, add these constants immediately after `CODE_REVIEW_STEP_2_INITIAL_PROMPT`:
...
- [x] Subtask 4.5. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/code-review/codeReviewWorkflow.ts`, update Step 2 `promptTemplates` from `[CODE_REVIEW_STEP_2_INITIAL_PROMPT]` to exactly:
...
- [x] Subtask 5.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/correct-course/correctCourseWorkflow.ts`, replace `const CORRECT_COURSE_STEP_3_PROMPT_TEMPLATE: string = String.raw\`...\`` with four readable multiline template literal constants named exactly:
...
The base constant must be exactly:
...
The final constant must be exactly:
...
- [x] Subtask 5.2. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/correct-course/correctCourseWorkflow.ts`, delete these constants and helpers:
...
- [x] Subtask 5.3. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/correct-course/correctCourseWorkflow.ts`, replace the body of `buildStep3PromptSource(input: WorkflowPromptBuilderInput): WorkflowStepPromptSource` with this exact section-assembly shape:
...
- [x] Subtask 5.4. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/correct-course/correctCourseWorkflow.ts`, update Step 3 `promptTemplates` from `[CORRECT_COURSE_STEP_3_PROMPT_TEMPLATE]` to exactly:
...
- [x] Subtask 7.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/create-epics/createEpicsToolSchemas.ts`, update the shared/default tool-schema wiring exactly as follows.
...
- [x] Subtask 7.2. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/create-epics/createEpicsWorkflow.ts`, add `WorkflowPromptBuilderInput` to the existing import from `../../types`.
  - Allowed files:
...
- [x] Subtask 7.4. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/create-epics/createEpicsWorkflow.ts`, replace `function buildStep2PromptSource(): WorkflowStepPromptSource` with this exact signature and section-assembly body:
...
- [x] Subtask 7.5. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/create-epics/createEpicsWorkflow.ts`, update Step 2 `promptTemplates` from `[CREATE_EPICS_STEP_2_PROMPT_TEMPLATE]` to exactly:
...
- [x] Subtask 7.6. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/create-epics/__tests__/createEpicsToolSchemas.test.ts`, update `STEP_2_TOOL_NAMES` to exactly:
...
- [x] Subtask 8.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/create-story/__tests__/createStoryWorkflow.test.ts`, inside `expectNoCreateStoryWorkflowPromptTokens(prompt: string): void`, after the existing raw workflow-token loop, add these exact assertions:
...
- [x] Subtask 9.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/dev-story/devStoryWorkflow.ts`, replace `DEV_STORY_STEP_2_PROMPT_TEMPLATE` with these exact constants. Preserve the static AI-facing Step 2 prompt text and separate the generated current-task detail from the static prompt template.
...
- [x] Subtask 9.2. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/dev-story/devStoryWorkflow.ts`, replace the initial-branch return in `buildStep2PromptSource(input: WorkflowPromptBuilderInput): WorkflowStepPromptSource` with this exact shape:
...
- [x] Subtask 9.3. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/dev-story/devStoryWorkflow.ts`, update Step 2 `promptTemplates` from `[DEV_STORY_STEP_2_PROMPT_TEMPLATE]` to `[DEV_STORY_STEP_2_STATIC_PROMPT_TEMPLATE]`. Do not add generated `currentTaskDetail` to `promptTemplates`. Remove any now-unused `DEV_STORY_STEP_2_PROMPT_TEMPLATE` reference.
  - Allowed files:
    - `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/dev-story/devStoryWorkflow.ts`

- [x] Subtask 9.4. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/dev-story/__tests__/devStoryWorkflow.test.ts`, inside `expectNoDevStoryWorkflowPromptTokens(prompt: string): void`, after the existing raw workflow-token loop, add these exact assertions:
...
- [x] Subtask 10.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, delete these constants:
...
- [x] Subtask 10.2. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, delete these constants:
...
- [x] Subtask 10.3. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, replace the body of `buildStep2PromptSource(input: WorkflowPromptBuilderInput): WorkflowStepPromptSource` with this exact section-assembly shape:
...
- [x] Subtask 10.4. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, replace the body of `buildStep3PromptSource(input: WorkflowPromptBuilderInput): WorkflowStepPromptSource` with this exact section-assembly shape:
...
- [x] Subtask 10.5. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, replace the body of `buildStep4PromptSource(input: WorkflowPromptBuilderInput): WorkflowStepPromptSource` with this exact section-assembly shape:
...
- [x] Subtask 10.6. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, replace the body of `buildStep5PromptSource(input: WorkflowPromptBuilderInput): WorkflowStepPromptSource` with this exact section-assembly shape:
...
- [x] Subtask 10.7. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, update Step 2 `promptTemplates` to exactly:
...
- [x] Subtask 10.8. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, update Step 3, Step 4, and Step 5 `promptTemplates` to exactly:
...
- [x] Subtask 10.9. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, in `expectNoPiPlanningWorkflowPromptTokens(prompt: string): void`, append these exact entries to `forbiddenTokens` after `"{workflow.target_story}"`:
...
- [x] Subtask 10.11. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, after test `renders Step 2 through Step 5 prompts with required workflow value references and no backend-only tools`, add a new test named `renders Step 2 optional context sections only when backing values are present`. The test must build these three prompts:
...
- [x] Subtask 11.1. In `/Users/robertboston/Documents/Cline Extension/cline/src/core/prompts/system-prompt/__tests__/integration.test.ts`, make all prompt-projection and tool-projection fallout updates for this file exactly as follows.

Inside test `projects active brainstorming Step 3 suggest tools into native GPT-5 prompts`, replace the `approvedStep3ToolNames` array with this exact array:
...
In the same test, add this exact assertion after `expect(nativeToolNames).to.not.include("set_workflow_values")`:

```ts
				expect(nativeToolNames).to.not.include("append_brainstorming_selected_technique")
```

Inside test `projects active brainstorming Step 3 choose and random tools into native GPT-5 prompts`, replace the `approvedStep3ToolNames` array with this exact array:
...
- [x] Subtask 12.4. Run this exact lint command:
...
- [x] Subtask 12.5. Run this exact package command:
...
- [x] Subtask 12.6. Run this exact scope-diff command:
...
src/core/task/workflow-runtime/workflow-modules/brainstorming/brainstormingToolSchemas.ts
src/core/task/workflow-runtime/workflow-modules/brainstorming/__tests__/brainstormingWorkflow.test.ts
src/core/task/workflow-runtime/workflow-modules/brainstorming/__tests__/brainstormingToolSchemas.test.ts
...
- [x] Subtask 12.7. Run this exact untracked-file command:
...
| Task 3 | brainstorming requirements Step 3 | `brainstormingWorkflow.ts`; `brainstormingWorkflow.test.ts`; `brainstormingToolSchemas.ts`; `brainstormingToolSchemas.test.ts` | Task owns Subtasks 3.1 through 3.6 | Live Step 3 prompt builder, promptTemplates, model-facing tool schemas, and tests verified | Subtasks remove obsolete two-variant prompt constants and Step 3 append-tool exposure | 11.1, 12.1, 12.2, 12.3 |
| 3.1 | brainstorming requirements Step 3 source prompt | `brainstormingWorkflow.ts` | `BRAINSTORMING_STEP_3_INTRO_PROMPT_TEMPLATE`, `BRAINSTORMING_STEP_3_SUGGEST_TECHNIQUE_PROMPT_TEMPLATE`, `BRAINSTORMING_STEP_3_FACILITATION_PROMPT_TEMPLATE` | Source document Step 3 prompt has an always-shown intro, one middle conditional segment for `BrainstormingSelectedApproach.Suggest`, and an always-shown facilitation segment | Deletes obsolete `STEP_3_SHARED_FACILITATION_PROMPT`, `BRAINSTORMING_STEP_3_SUGGEST_PROMPT_TEMPLATE`, and `BRAINSTORMING_STEP_3_STANDARD_PROMPT_TEMPLATE` | 3.4, 12.1, 12.2 |
| 3.2 | brainstorming requirements Step 3 conditional prompt ordering | `brainstormingWorkflow.ts` | `buildStep3PromptSource`, `readSelectedApproach`, `BrainstormingSelectedApproach.Suggest` | Step 3 receives `WorkflowPromptBuilderInput`, assembles static sections, and returns `current_step_instruction_template` | No local workflow-value substitution added; no full-prompt branch constants preserved | 3.4, 12.1, 12.3 |
| 3.3 | brainstorming requirements Step 3 prompt template inventory | `brainstormingWorkflow.ts` | Step 3 `promptTemplates` | Step 3 templates list static sections returned by `buildStep3PromptSource` | Obsolete full-prompt template references removed | 12.1, 12.3 |
| 3.4 | brainstorming testing requirements | `brainstormingWorkflow.test.ts` | Test `builds Step 3 prompt and tool variants and routes workflow progress decisions` | Existing helper renders through shared renderer for suggest, choose, and random workflow values | Stale assertions for `append_brainstorming_selected_technique`, `set_workflow_values`, and `Use the already selected brainstorming technique recorded` removed | 12.1 |
| 3.5 | brainstorming Step 3 model-facing tool schema | `brainstormingToolSchemas.ts` | `buildBrainstormingAppendSelectedTechniqueToolSchema`, `buildBrainstormingStep3SuggestToolSchemas`, `buildBrainstormingStep3ChooseOrRandomToolSchemas` | Step 3 model-facing schema includes `apply_patch` and excludes `append_brainstorming_selected_technique` | Deletes module-local append schema builder; shared backend append handler and contracts remain untouched | 3.6, 11.1, 12.1, 12.2, 12.3 |
| 3.6 | brainstorming Step 3 tool-schema tests | `brainstormingToolSchemas.test.ts` | Import list, test `builds schemas for the Phase 3 suggestion-only brainstorming tools`, test `exposes exact Step 3 and Step 4 active-step tool schemas`, test `keeps runtime-owned artifact tools out of model-facing brainstorming schemas` | Existing tests directly call `buildBrainstormingStep3ToolSchemas` for suggest, choose, and random approaches | Removes obsolete append schema import and assertions; adds non-model-facing append guard | 12.1 |
...
| Task 11 | action-plan-guide file-owned fallout requirement; prompt projection requirements | `integration.test.ts` | Task owns Subtask 11.1 | Live integration projection tests for brainstorming, create-architecture, create-epics, and pi-planning verified | Consolidates all `integration.test.ts` revisions into one file-owned subtask | 12.1 |
| 11.1 | brainstorming/create-architecture/create-epics/pi-planning prompt projection fallout | `integration.test.ts` | Tests `projects active brainstorming Step 3 suggest tools into native GPT-5 prompts`, `projects active brainstorming Step 3 choose and random tools into native GPT-5 prompts`, `projects create-architecture current step details into the full-turn input payload only`, `projects active create-epics Step 2 tools into native GPT-5 prompts`, `projects create-epics current step details into the full-turn input payload only`, and `projects pi-planning current step details into the full-turn input payload only` | Full-turn payload, system-prompt negative assertions, and native tool list assertions match revised module contracts | Removes stale brainstorming append-tool integration assertions plus stale Step 3, Step 2, and forbidden `apply_patch` integration assertions | 12.1 |
...
| 12.1 | module-build-guide validation expectations | Validation command | Focused unit/integration test paths | Command paths exist in repo | Includes `brainstormingToolSchemas.test.ts`, `integration.test.ts`, and create-epics schema tests | 12.1 |
| 12.2 | approved static guard requirements | Validation command | `rg` guard pattern and target files | Guard avoids `current_story_task_id` false positive through word-boundary pattern, includes every changed workflow file, checks `brainstormingToolSchemas.ts`, and checks double-quoted or backtick one-line `\n` prompt-body regressions | Guard includes acceptance-audit-review, blind-review, and brainstorming Step 3 append-tool fallout | 12.2 |
...
| 12.6 | action-plan-guide scope diff | Validation command | `git diff --name-only` allowlist | Allowlist includes every code/test file touched by subtasks | Brainstorming schema, integration, and create-epics schema fallout included | 12.6 |
