...
[x] Task 15. Update PI Planning Step 1 values, form panels, selected-story metadata derivation, and routing through existing runtime-owned story actions.
...
- `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`
    ...
    [x] Subtask 15.3: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, update the `../../types` import to include exact symbols `WorkflowDecisionAction` and `WorkflowFormContinuationReplacement`.

    [x] Subtask 15.4: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add import `WorkflowArtifactFamily` from `../../artifactFamilies`.

    [x] Subtask 15.5: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add type import `WorkflowStoryStatus` from `../../storyArtifacts`; do not import `parseWorkflowStoryIndexJson` or `WorkflowStoryIndexEntry` for selected-story derivation because selected-story derivation intentionally performs only loose JSON reads before runtime-owned `validate_story_index_entry` performs strict story-index parsing.
    ...
    [x] Subtask 15.8: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, delete constant `STEP_1_REQUIRED_CONTEXT_PANEL_ID`.

    [x] Subtask 15.9: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add constants `STEP_1_EDIT_INTENT_PANEL_ID = "step-1-edit-intent-panel"` and `STEP_1_SELECT_STORY_PANEL_ID = "step-1-select-story-panel"`.

    [x] Subtask 15.10: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add `function buildRuntimeRoutedTransition(): WorkflowFormPanelDefinition["transition"]` returning exactly `{ type: "runtime_routed" }`.

    [x] Subtask 15.11: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add `function buildOption(value: string): WorkflowFormOptionDefinition` returning exactly `{ value, label: value }`.
    ...
    [x] Subtask 15.31: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, in `deriveSelectedStoryValuesFromForm(...)`, assign `const selectedStoryFileName = selectedStory.story_file_name`; require `typeof selectedStoryFileName === "string"`; otherwise return `{ kind: "failed", errorMessage: "I could not read or parse the selected story index before resolving the target story." }`. Do not trim, canonicalize, or return a filename-specific error; runtime-owned `validate_story_index_entry` remains responsible for strict story-index parsing.
    ...
    [x] Subtask 15.37: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add `function buildStep1TargetEpicPanel(): WorkflowFormPanelDefinition` returning exact panel fields `panelId: STEP_1_TARGET_EPIC_PANEL_ID`, `title: "Target Epic"`, `promptMarkdown: "Which epic are we working on during this workflow?"`, one dropdown field with `key` and `workflowValueKey` equal to `PiPlanningWorkflowValueKey.EpicIdentity`, `label: "Target Epic"`, `required: true`, `allowedValueType: "string"`, `resetValueKeysOnChange: [PiPlanningWorkflowValueKey.TargetEpic, PiPlanningWorkflowValueKey.StoriesIndex, PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart, PiPlanningWorkflowValueKey.EditIntent, PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext]`, `jsonOptionsSource` equal to `{ root: { kind: "selected_project_root" }, sourcePathSegments: ["planning", "Epics.index.json"], itemsPath: "epics", valueProperty: "identity", labelTemplate: "Epic {identity}: {title}", descriptionTemplate: "Story index generated: {story-index-generated}" }`, `allowedActions: ["submit"]`, `actionLabels: { submit: "Continue" }`, and `transition: buildRuntimeRoutedTransition()`.

    [x] Subtask 15.38: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add `function buildStep1EditIntentPanel(): WorkflowFormPanelDefinition` returning exact panel fields `panelId: STEP_1_EDIT_INTENT_PANEL_ID`, `title: "Provide Edit Intent"`, `promptMarkdown: "It looks like the selected epic already has a story index file with generated story files. Please select one of the following options:"`, one dropdown field with `key` and `workflowValueKey` equal to `PiPlanningWorkflowValueKey.EditIntent`, `label: "select one"`, `required: true`, `allowedValueType: "string"`, `resetValueKeysOnChange: [PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext]`, `options: [buildOption(PiPlanningEditIntent.CompleteInitialStoryBuildout), buildOption(PiPlanningEditIntent.EditExistingStoryFile)]`, `allowedActions: ["submit", "back"]`, `actionLabels: { submit: "Continue", back: "Back" }`, `backDestinationPanelId: STEP_1_TARGET_EPIC_PANEL_ID`, `backStaleValueKeysToClear: [PiPlanningWorkflowValueKey.EditIntent, PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext]`, and `transition: buildRuntimeRoutedTransition()`.

    [x] Subtask 15.39: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add `function buildStep1SelectStoryPanel(): WorkflowFormPanelDefinition` returning exact panel fields `panelId: STEP_1_SELECT_STORY_PANEL_ID`, `title: "Select Story"`, `promptMarkdown: "Which story would you like to edit?"`, one dropdown field with `key` and `workflowValueKey` equal to `PiPlanningWorkflowValueKey.SelectedStoryIdentity`, `label: "Select Story"`, `required: true`, `allowedValueType: "string"`, `resetValueKeysOnChange: [PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext]`, `jsonOptionsSource` equal to `{ root: { kind: "selected_project_root" }, sourcePathSegments: ["implementation", "epic-{workflow.epic_identity}-stories.index.json"], itemsPath: "stories", valueProperty: "story_identity", labelTemplate: "Story {story_identity}: {story_file_name}" }`, `allowedActions: ["submit", "back"]`, `actionLabels: { submit: "Continue", back: "Back" }`, `backDestinationPanelId: STEP_1_EDIT_INTENT_PANEL_ID`, `backStaleValueKeysToClear: [PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext]`, and `transition: buildRuntimeRoutedTransition()`.

    [x] Subtask 15.40: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add `function buildStep1AdditionalContextPanel(): WorkflowFormPanelDefinition` returning exact panel fields `panelId: STEP_1_ADDITIONAL_CONTEXT_PANEL_ID`, `title: "Additional Context"`, `promptMarkdown: "If you'd like to include any other files as workflow context please provide their full file paths below."`, one large-text field with `key` and `workflowValueKey` equal to `PiPlanningWorkflowValueKey.AdditionalContext`, `label: "Additional context file paths"`, `required: false`, `allowedValueType: "string"`, `presentation: { textareaSize: "large" }`, `allowedActions: ["submit"]`, `actionLabels: { submit: "Continue" }`, no `backDestinationPanelId`, and `transition: buildTerminalTransition()`.
    ...
    [x] Subtask 15.42: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, add `function buildStep1ContinuationReplacement(panel: WorkflowFormPanelDefinition): WorkflowFormContinuationReplacement` returning exact object `{ panel, data: {} }`.

    [x] Subtask 15.43: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts`, in `buildStep1DecisionTree()`, update route `step-1-render-input-form` so its `followingBranchId` is exactly `"step-1-await-target-epic-panel"`.
...
- `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningToolSchemas.test.ts`
    ...
    [x] Subtask 17.3: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, replace the Required Context panel test with a test asserting `getStep1InputForm().panels["step-1-required-context-panel"] === undefined`.

    [x] Subtask 17.4: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, update Panel A coverage to assert exact panel id `step-1-target-epic-panel`, title `Target Epic`, promptMarkdown `Which epic are we working on during this workflow?`, actions `["submit"]`, labels `{ submit: "Continue" }`, exact dropdown field object `{ key: PiPlanningWorkflowValueKey.EpicIdentity, workflowValueKey: PiPlanningWorkflowValueKey.EpicIdentity, kind: "dropdown", label: "Target Epic", required: true, allowedValueType: "string", resetValueKeysOnChange: [PiPlanningWorkflowValueKey.TargetEpic, PiPlanningWorkflowValueKey.StoriesIndex, PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart, PiPlanningWorkflowValueKey.EditIntent, PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext], jsonOptionsSource: { root: { kind: "selected_project_root" }, sourcePathSegments: ["planning", "Epics.index.json"], itemsPath: "epics", valueProperty: "identity", labelTemplate: "Epic {identity}: {title}", descriptionTemplate: "Story index generated: {story-index-generated}" } }`, and exact `transition` deep-equals `{ type: "runtime_routed" }`.

    [x] Subtask 17.5: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Panel B coverage asserting exact panel id `step-1-edit-intent-panel`, title `Provide Edit Intent`, promptMarkdown `It looks like the selected epic already has a story index file with generated story files. Please select one of the following options:`, exact dropdown field object `{ key: PiPlanningWorkflowValueKey.EditIntent, workflowValueKey: PiPlanningWorkflowValueKey.EditIntent, kind: "dropdown", label: "select one", required: true, allowedValueType: "string", resetValueKeysOnChange: [PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext], options: [{ value: "Complete initial story buildout", label: "Complete initial story buildout" }, { value: "edit existing story file", label: "edit existing story file" }] }`, actions `["submit", "back"]`, labels `{ submit: "Continue", back: "Back" }`, `backDestinationPanelId: "step-1-target-epic-panel"`, `backStaleValueKeysToClear` deep-equals `[PiPlanningWorkflowValueKey.EditIntent, PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext]`, and transition `{ type: "runtime_routed" }`.

    [x] Subtask 17.6: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Panel C coverage asserting exact panel id `step-1-select-story-panel`, title `Select Story`, promptMarkdown `Which story would you like to edit?`, exact dropdown field object `{ key: PiPlanningWorkflowValueKey.SelectedStoryIdentity, workflowValueKey: PiPlanningWorkflowValueKey.SelectedStoryIdentity, kind: "dropdown", label: "Select Story", required: true, allowedValueType: "string", resetValueKeysOnChange: [PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext], jsonOptionsSource: { root: { kind: "selected_project_root" }, sourcePathSegments: ["implementation", "epic-{workflow.epic_identity}-stories.index.json"], itemsPath: "stories", valueProperty: "story_identity", labelTemplate: "Story {story_identity}: {story_file_name}" } }`, then narrowing with `expect(field.kind).to.equal("dropdown")` and `if (field.kind !== "dropdown") { throw new Error(\`Expected dropdown field, received ${field.kind}.\`) }`, assigning `const jsonOptionsSource = field.jsonOptionsSource`, asserting `jsonOptionsSource !== undefined`, narrowing with `if (jsonOptionsSource === undefined) { throw new Error("Expected story-selection field to define jsonOptionsSource.") }`, asserting `jsonOptionsSource.descriptionTemplate === undefined`, actions `["submit", "back"]`, labels `{ submit: "Continue", back: "Back" }`, `backDestinationPanelId: "step-1-edit-intent-panel"`, `backStaleValueKeysToClear` deep-equals `[PiPlanningWorkflowValueKey.SelectedStoryIdentity, PiPlanningWorkflowValueKey.SelectedStoryFileName, PiPlanningWorkflowValueKey.SelectedStoryStatus, PiPlanningWorkflowValueKey.TargetStory, PiPlanningWorkflowValueKey.AdditionalContext]`, and transition `{ type: "runtime_routed" }`.

    [x] Subtask 17.7: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, update Additional Context coverage to call it Panel D and assert exact panel id `step-1-additional-context-panel`, title `Additional Context`, promptMarkdown `If you'd like to include any other files as workflow context please provide their full file paths below.`, exact large-text field object `{ key: PiPlanningWorkflowValueKey.AdditionalContext, workflowValueKey: PiPlanningWorkflowValueKey.AdditionalContext, kind: "large_text", label: "Additional context file paths", required: false, allowedValueType: "string", presentation: { textareaSize: "large" } }`, actions `["submit"]`, labels `{ submit: "Continue" }`, `backDestinationPanelId === undefined`, `backStaleValueKeysToClear === undefined`, and exact transition deep-equals `{ type: "conditional", conditionSourceKey: "__terminal__", branches: [], defaultTerminal: true }`.
    ...
    [x] Subtask 17.16: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 Panel A submission route coverage for route `step-1-derive-selected-epic-values` in branch `step-1-await-target-epic-panel`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-target-epic-panel", workflowValues: SAMPLE_WORKFLOW_VALUES, step: getStep("step-1"), triggerEvent: buildWorkflowFormPanelSubmittedEvent("step-1-target-epic-panel", "submit") })`, asserting action kind `run_deterministic_procedure`, and asserting `followingBranchId === "step-1-route-after-target-epic-panel"`.

    [x] Subtask 17.17: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 existing-index route coverage for route `step-1-continue-to-edit-intent-panel` in branch `step-1-route-after-target-epic-panel`, narrowing with `expect(route.trigger.kind).to.equal("session_predicate")` and `if (route.trigger.kind !== "session_predicate") { throw new Error(\`Expected session_predicate trigger, received ${route.trigger.kind}.\`) }`, asserting `route.trigger.matches({ activeBranchId: "step-1-route-after-target-epic-panel", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: "/tmp/pi-planning-project/implementation/epic-1-stories.index.json" }, step: getStep("step-1") }) === true`, using `await expectContinueWorkflowFormAction(route.action, "step-1-edit-intent-panel")`, and asserting `followingBranchId === "step-1-await-edit-intent-panel"`.

    [x] Subtask 17.18: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 missing-index route coverage for route `step-1-continue-to-additional-context-after-new-index-epic` in branch `step-1-route-after-target-epic-panel`, narrowing with `expect(route.trigger.kind).to.equal("session_predicate")` and `if (route.trigger.kind !== "session_predicate") { throw new Error(\`Expected session_predicate trigger, received ${route.trigger.kind}.\`) }`, constructing `const workflowValues: WorkflowValues = { ...SAMPLE_WORKFLOW_VALUES }`, deleting `workflowValues[PiPlanningWorkflowValueKey.StoriesIndex]`, asserting `route.trigger.matches({ activeBranchId: "step-1-route-after-target-epic-panel", workflowValues, step: getStep("step-1") }) === true`, using `await expectContinueWorkflowFormAction(route.action, "step-1-additional-context-panel")`, and asserting `followingBranchId === "step-1-await-final-form-submit"`.

    [x] Subtask 17.19: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 Panel B submission route coverage for route `step-1-route-after-edit-intent` in branch `step-1-await-edit-intent-panel`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-edit-intent-panel", workflowValues: SAMPLE_WORKFLOW_VALUES, step: getStep("step-1"), triggerEvent: buildWorkflowFormPanelSubmittedEvent("step-1-edit-intent-panel", "submit") })`, asserting action deep-equals `{ kind: "no_op" }`, and asserting `followingBranchId === "step-1-route-after-edit-intent-panel"`.

    [x] Subtask 17.20: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 Panel B complete-initial-buildout route coverage for route `step-1-continue-to-additional-context-after-complete-initial-buildout` in branch `step-1-route-after-edit-intent-panel`, narrowing with `expect(route.trigger.kind).to.equal("session_predicate")` and `if (route.trigger.kind !== "session_predicate") { throw new Error(\`Expected session_predicate trigger, received ${route.trigger.kind}.\`) }`, asserting `route.trigger.matches({ activeBranchId: "step-1-route-after-edit-intent-panel", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.EditIntent]: "Complete initial story buildout" }, step: getStep("step-1") }) === true`, using `await expectContinueWorkflowFormAction(route.action, "step-1-additional-context-panel")`, and asserting `followingBranchId === "step-1-await-final-form-submit"`.

    [x] Subtask 17.21: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 Panel B edit-existing-story route coverage for route `step-1-continue-to-select-story-panel` in branch `step-1-route-after-edit-intent-panel`, narrowing with `expect(route.trigger.kind).to.equal("session_predicate")` and `if (route.trigger.kind !== "session_predicate") { throw new Error(\`Expected session_predicate trigger, received ${route.trigger.kind}.\`) }`, asserting `route.trigger.matches({ activeBranchId: "step-1-route-after-edit-intent-panel", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.EditIntent]: "edit existing story file" }, step: getStep("step-1") }) === true`, using `await expectContinueWorkflowFormAction(route.action, "step-1-select-story-panel")`, and asserting `followingBranchId === "step-1-await-select-story-panel"`.

    [x] Subtask 17.22: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 Panel C story-selection route coverage for route `step-1-derive-selected-story-values` in branch `step-1-await-select-story-panel`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-select-story-panel", workflowValues: SAMPLE_WORKFLOW_VALUES, step: getStep("step-1"), triggerEvent: buildWorkflowFormPanelSubmittedEvent("step-1-select-story-panel", "submit") })`, asserting action kind `run_deterministic_procedure`, and asserting `followingBranchId === "step-1-continue-to-additional-context-after-story-selection"`.
    ...
    [x] Subtask 17.24: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 final-form route coverage for route `step-1-transition-to-step-2-after-new-index-epic`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-final-form-submit", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart]: false }, step: getStep("step-1"), triggerEvent: buildWorkflowFormCompletedEvent("step-1-input-form") })`, using `expectEventPredicateDoesNotMatch({ route, activeBranchId: "step-1-await-final-form-submit", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart]: true }, step: getStep("step-1"), triggerEvent: buildWorkflowFormCompletedEvent("step-1-input-form") })`, and asserting action deep-equals `{ kind: "transition_step", target: { kind: "entry_branch", stepNumber: 2 } }`.

    [x] Subtask 17.25: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 final-form route coverage for route `step-1-transition-to-step-2-after-complete-initial-buildout`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-final-form-submit", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.EditIntent]: "Complete initial story buildout" }, step: getStep("step-1"), triggerEvent: buildWorkflowFormCompletedEvent("step-1-input-form") })`, using `expectEventPredicateDoesNotMatch({ route, activeBranchId: "step-1-await-final-form-submit", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.EditIntent]: "edit existing story file" }, step: getStep("step-1"), triggerEvent: buildWorkflowFormCompletedEvent("step-1-input-form") })`, and asserting action deep-equals `{ kind: "transition_step", target: { kind: "entry_branch", stepNumber: 2 } }`.

    [x] Subtask 17.26: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add Step 1 final-form route coverage for route `step-1-generate-missing-story-files-before-edit`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-final-form-submit", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.EditIntent]: "edit existing story file" }, step: getStep("step-1"), triggerEvent: buildWorkflowFormCompletedEvent("step-1-input-form") })`, using `expectEventPredicateDoesNotMatch({ route, activeBranchId: "step-1-await-final-form-submit", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.EditIntent]: "Complete initial story buildout" }, step: getStep("step-1"), triggerEvent: buildWorkflowFormCompletedEvent("step-1-input-form") })`, using `expectExecuteToolBackedOperationAction(route.action)`, asserting `instruction.toolName === ClineDefaultTool.GENERATE_STORY_FILES`, calling `instruction.buildStatusDefinition(...)`, `instruction.buildToolExecutionRequest(...)`, and `instruction.evaluateToolExecutionResult(...)` with exact `toolBackedOperationSession` fixture `{ sessionId: "pi-planning-generate-missing-story-files", sourceRoute: { branchId: "step-1-await-final-form-submit", routeId: "step-1-generate-missing-story-files-before-edit" }, triggerSource: "execute_tool_backed_operation", owner: { kind: "workflow_step", workflowName: "pi-planning", stepNumber: 1 }, state: "pending" }`, calling `instruction.buildToolExecutionRequest(...)` with exact `activeWorkflowSession: createSession({ ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.EditIntent]: "edit existing story file", [PiPlanningWorkflowValueKey.EpicIdentity]: "1" })`, calling `instruction.evaluateToolExecutionResult(...)` with exact result args `{}`, asserting exact status definition `{ title: "Generate Missing Story Files", pendingLabel: "Generating missing story files", successLabel: "Generated missing story files", failureLabel: "Failed to generate missing story files" }`, exact tool execution request `{ toolName: ClineDefaultTool.GENERATE_STORY_FILES, toolInput: {}, toolParams: { epic_identity: "1" } }`, exact evaluation result `{ succeeded: true }`, and `followingBranchId === "step-1-await-missing-story-generation"`.

    [x] Subtask 17.27: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add generation-success route coverage for route `step-1-route-target-story-status-after-missing-story-generation`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-missing-story-generation", workflowValues: SAMPLE_WORKFLOW_VALUES, step: getStep("step-1"), triggerEvent: buildToolBackedOperationSucceededEvent({ branchId: "step-1-await-final-form-submit", routeId: "step-1-generate-missing-story-files-before-edit" }) })`, asserting action deep-equals `{ kind: "no_op" }`, and asserting `followingBranchId === "step-1-route-target-story-status"`.

    [x] Subtask 17.28: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add generation-failure route coverage for route `step-1-fail-after-missing-story-generation`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-missing-story-generation", workflowValues: SAMPLE_WORKFLOW_VALUES, step: getStep("step-1"), triggerEvent: buildToolBackedOperationFailedEvent({ branchId: "step-1-await-final-form-submit", routeId: "step-1-generate-missing-story-files-before-edit" }) })`, and asserting action deep-equals `{ kind: "terminal_error", errorMessage: "Failed to generate missing story files" }`.

    [x] Subtask 17.29: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add draft validation route coverage for route `step-1-validate-draft-story-index-entry` in branch `step-1-route-target-story-status`, narrowing with `expect(route.trigger.kind).to.equal("session_predicate")` and `if (route.trigger.kind !== "session_predicate") { throw new Error(\`Expected session_predicate trigger, received ${route.trigger.kind}.\`) }`, asserting `route.trigger.matches({ activeBranchId: "step-1-route-target-story-status", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.SelectedStoryStatus]: "draft" }, step: getStep("step-1") }) === true`, using `expectValidateStoryIndexEntryAction(route.action)`, asserting exact action fields `{ kind: "validate_story_index_entry", storyIndexWorkflowValueKey: PiPlanningWorkflowValueKey.StoriesIndex, storyIdentityWorkflowValueKey: PiPlanningWorkflowValueKey.SelectedStoryIdentity, storyFilenameWorkflowValueKey: PiPlanningWorkflowValueKey.SelectedStoryFileName, requiredStoryType: "primary", requiredStatus: "draft", missingOrMalformedIndexErrorMessage: "I could not read or parse the selected story index before resolving the target story.", missingEntryErrorMessage: "The selected story was not found in the selected story index.", invalidEntryErrorMessage: "I could not read or parse the selected story index before resolving the target story." }`, and asserting `followingBranchId === "step-1-resolve-draft-target-story"`.

    [x] Subtask 17.30: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add backlog validation route coverage for route `step-1-validate-backlog-story-index-entry` in branch `step-1-route-target-story-status`, narrowing with `expect(route.trigger.kind).to.equal("session_predicate")` and `if (route.trigger.kind !== "session_predicate") { throw new Error(\`Expected session_predicate trigger, received ${route.trigger.kind}.\`) }`, asserting `route.trigger.matches({ activeBranchId: "step-1-route-target-story-status", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.SelectedStoryStatus]: "backlog" }, step: getStep("step-1") }) === true`, using `expectValidateStoryIndexEntryAction(route.action)`, asserting exact action fields `{ kind: "validate_story_index_entry", storyIndexWorkflowValueKey: PiPlanningWorkflowValueKey.StoriesIndex, storyIdentityWorkflowValueKey: PiPlanningWorkflowValueKey.SelectedStoryIdentity, storyFilenameWorkflowValueKey: PiPlanningWorkflowValueKey.SelectedStoryFileName, requiredStoryType: "primary", requiredStatus: "backlog", missingOrMalformedIndexErrorMessage: "I could not read or parse the selected story index before resolving the target story.", missingEntryErrorMessage: "The selected story was not found in the selected story index.", invalidEntryErrorMessage: "I could not read or parse the selected story index before resolving the target story." }`, and asserting `followingBranchId === "step-1-resolve-backlog-target-story"`.

    [x] Subtask 17.31: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add unsupported status route coverage for route `step-1-fail-unsupported-selected-story-status` in branch `step-1-route-target-story-status`, narrowing with `expect(route.trigger.kind).to.equal("session_predicate")` and `if (route.trigger.kind !== "session_predicate") { throw new Error(\`Expected session_predicate trigger, received ${route.trigger.kind}.\`) }`, asserting `route.trigger.matches({ activeBranchId: "step-1-route-target-story-status", workflowValues: { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.SelectedStoryStatus]: "review" }, step: getStep("step-1") }) === true`, and asserting action deep-equals `{ kind: "terminal_error", errorMessage: "The selected story has an unsupported story status." }`.

    [x] Subtask 17.32: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add draft target-story resolution route coverage for route `step-1-resolve-draft-target-story`, asserting trigger deep-equals `{ kind: "always" }`, using `expectResolveExistingProjectArtifactAction(route.action)`, asserting exact action fields `{ kind: "resolve_existing_project_artifact", artifactFamily: "story", artifactIdentityWorkflowValueKey: PiPlanningWorkflowValueKey.SelectedStoryIdentity, projectSubfolderSegments: ["implementation", "drafts"], outputWorkflowValueKey: PiPlanningWorkflowValueKey.TargetStory, missingArtifactErrorMessage: "The target story path does not exist." }`, and asserting `followingBranchId === "step-1-await-target-story-resolution"`.

    [x] Subtask 17.33: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add backlog target-story resolution route coverage for route `step-1-resolve-backlog-target-story`, asserting trigger deep-equals `{ kind: "always" }`, using `expectResolveExistingProjectArtifactAction(route.action)`, asserting exact action fields `{ kind: "resolve_existing_project_artifact", artifactFamily: "story", artifactIdentityWorkflowValueKey: PiPlanningWorkflowValueKey.SelectedStoryIdentity, projectSubfolderSegments: ["implementation", "stories-backlog"], outputWorkflowValueKey: PiPlanningWorkflowValueKey.TargetStory, missingArtifactErrorMessage: "The target story path does not exist." }`, and asserting `followingBranchId === "step-1-await-target-story-resolution"`.

    [x] Subtask 17.34: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add target-story persistence route coverage for route `step-1-transition-to-step-6-after-target-story-resolution`, using `expectEventPredicateMatches({ route, activeBranchId: "step-1-await-target-story-resolution", workflowValues: SAMPLE_WORKFLOW_VALUES, step: getStep("step-1"), triggerEvent: buildWorkflowValuesPersistedEvent([PiPlanningWorkflowValueKey.TargetStory]) })`, and asserting action deep-equals `{ kind: "transition_step", target: { kind: "entry_branch", stepNumber: 6 } }`.

    [x] Subtask 17.35: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, update selected epic derivation coverage to assert existing-index write set exactly equals `{ [PiPlanningWorkflowValueKey.TargetEpic]: "Epic 1: Existing story index epic", [PiPlanningWorkflowValueKey.StoriesIndex]: join(projectRoot, "implementation", "epic-1-stories.index.json"), [PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart]: true }` and missing-index write set exactly equals `{ [PiPlanningWorkflowValueKey.TargetEpic]: "Epic 2: New story index epic", [PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart]: false }`.

    [x] Subtask 17.36: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add selected-story derivation success coverage that retrieves `const route = findRoute("step-1", "step-1-await-select-story-panel", "step-1-derive-selected-story-values")`, narrows with `expect(route.action.kind).to.equal("run_deterministic_procedure")` and `if (route.action.kind !== "run_deterministic_procedure") { throw new Error(\`Expected run_deterministic_procedure, received ${route.action.kind}.\`) }`, then uses a fixture created with `const fixtureRoot = await mkdtemp(join(tmpdir(), "pi-planning-workflow-"))`, `const implementationDir = join(fixtureRoot, "implementation")`, `await mkdir(implementationDir, { recursive: true })`, and `const storiesIndexPath = join(implementationDir, "epic-1-stories.index.json")` inside a `try` block whose `finally` block calls `await rm(fixtureRoot, { recursive: true, force: true })`; write the story index with `await writeFile(storiesIndexPath, JSON.stringify({ version: 1, stories: [{ story_identity: "1.1", story_file_name: "Story-1-1.md", story_type: "primary", parent_story_identity: null, story_file_generated: true, status: "draft" }, { story_identity: "1.2", story_file_name: "Story-1-2.md", story_type: "primary", parent_story_identity: null, story_file_generated: true, status: "backlog" }] }), "utf8")`; call `await route.action.instruction.run(createSession({ ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: storiesIndexPath, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.1" }))` and `await route.action.instruction.run(createSession({ ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: storiesIndexPath, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.2" }))`; assert exact success writes `{ [PiPlanningWorkflowValueKey.SelectedStoryFileName]: "Story-1-1.md", [PiPlanningWorkflowValueKey.SelectedStoryStatus]: "draft" }` and `{ [PiPlanningWorkflowValueKey.SelectedStoryFileName]: "Story-1-2.md", [PiPlanningWorkflowValueKey.SelectedStoryStatus]: "backlog" }`.

    [x] Subtask 17.37: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add selected-story derivation failure coverage for missing `selected_story_identity` that retrieves `const route = findRoute("step-1", "step-1-await-select-story-panel", "step-1-derive-selected-story-values")`, narrows with `expect(route.action.kind).to.equal("run_deterministic_procedure")` and `if (route.action.kind !== "run_deterministic_procedure") { throw new Error(\`Expected run_deterministic_procedure, received ${route.action.kind}.\`) }`, constructs `const workflowValues: WorkflowValues = { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: "/tmp/pi-planning-project/implementation/epic-1-stories.index.json" }`, deletes `workflowValues[PiPlanningWorkflowValueKey.SelectedStoryIdentity]`, calls `await route.action.instruction.run(createSession(workflowValues))`, and asserts exact failed result `{ kind: "failed", errorMessage: "PI Planning requires a selected story identity before resolving the target story." }`.

    [x] Subtask 17.38: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add selected-story derivation failure coverage for missing `stories_index` that retrieves `const route = findRoute("step-1", "step-1-await-select-story-panel", "step-1-derive-selected-story-values")`, narrows with `expect(route.action.kind).to.equal("run_deterministic_procedure")` and `if (route.action.kind !== "run_deterministic_procedure") { throw new Error(\`Expected run_deterministic_procedure, received ${route.action.kind}.\`) }`, constructs `const workflowValues: WorkflowValues = { ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.1" }`, deletes `workflowValues[PiPlanningWorkflowValueKey.StoriesIndex]`, calls `await route.action.instruction.run(createSession(workflowValues))`, and asserts exact failed result `{ kind: "failed", errorMessage: "PI Planning requires a resolved stories_index path before resolving the target story." }`.

    [x] Subtask 17.39: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add selected-story derivation failure coverage for unreadable or malformed story index that retrieves `const route = findRoute("step-1", "step-1-await-select-story-panel", "step-1-derive-selected-story-values")`, narrows with `expect(route.action.kind).to.equal("run_deterministic_procedure")` and `if (route.action.kind !== "run_deterministic_procedure") { throw new Error(\`Expected run_deterministic_procedure, received ${route.action.kind}.\`) }`, then uses a fixture created with `const fixtureRoot = await mkdtemp(join(tmpdir(), "pi-planning-workflow-"))`, `const implementationDir = join(fixtureRoot, "implementation")`, `await mkdir(implementationDir, { recursive: true })`, and `const storiesIndexPath = join(implementationDir, "epic-1-stories.index.json")` inside a `try` block whose `finally` block calls `await rm(fixtureRoot, { recursive: true, force: true })`; write malformed JSON with `await writeFile(storiesIndexPath, "{ invalid json", "utf8")`; call `await route.action.instruction.run(createSession({ ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: storiesIndexPath, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.1" }))`; assert exact failed result `{ kind: "failed", errorMessage: "I could not read or parse the selected story index before resolving the target story." }`.

    [x] Subtask 17.40: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add selected-story derivation failure coverage for selected story identity absent from the index that retrieves `const route = findRoute("step-1", "step-1-await-select-story-panel", "step-1-derive-selected-story-values")`, narrows with `expect(route.action.kind).to.equal("run_deterministic_procedure")` and `if (route.action.kind !== "run_deterministic_procedure") { throw new Error(\`Expected run_deterministic_procedure, received ${route.action.kind}.\`) }`, then uses a fixture created with `const fixtureRoot = await mkdtemp(join(tmpdir(), "pi-planning-workflow-"))`, `const implementationDir = join(fixtureRoot, "implementation")`, `await mkdir(implementationDir, { recursive: true })`, and `const storiesIndexPath = join(implementationDir, "epic-1-stories.index.json")` inside a `try` block whose `finally` block calls `await rm(fixtureRoot, { recursive: true, force: true })`; write a valid index with `await writeFile(storiesIndexPath, JSON.stringify({ version: 1, stories: [{ story_identity: "1.9", story_file_name: "Story-1-9.md", story_type: "primary", parent_story_identity: null, story_file_generated: true, status: "draft" }] }), "utf8")`; call `await route.action.instruction.run(createSession({ ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: storiesIndexPath, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.1" }))`; assert exact failed result `{ kind: "failed", errorMessage: "The selected story was not found in the selected story index." }`.

    [x] Subtask 17.41: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add selected-story derivation failure coverage for unsupported entries that retrieves `const route = findRoute("step-1", "step-1-await-select-story-panel", "step-1-derive-selected-story-values")`, narrows with `expect(route.action.kind).to.equal("run_deterministic_procedure")` and `if (route.action.kind !== "run_deterministic_procedure") { throw new Error(\`Expected run_deterministic_procedure, received ${route.action.kind}.\`) }`, declares `const unsupportedCases = [{ story_type: "primary", status: "review" }, { story_type: "primary", status: "complete" }, { story_type: "remediation", status: "draft" }]`, and loops with `for (const caseValue of unsupportedCases)`; for each case, create a fresh fixture with `const fixtureRoot = await mkdtemp(join(tmpdir(), "pi-planning-workflow-"))`, `const implementationDir = join(fixtureRoot, "implementation")`, `await mkdir(implementationDir, { recursive: true })`, and `const storiesIndexPath = join(implementationDir, "epic-1-stories.index.json")` inside a `try` block whose `finally` block calls `await rm(fixtureRoot, { recursive: true, force: true })`; write `await writeFile(storiesIndexPath, JSON.stringify({ version: 1, stories: [{ story_identity: "1.1", story_file_name: "Story-1-1.md", story_type: caseValue.story_type, parent_story_identity: null, story_file_generated: true, status: caseValue.status }] }), "utf8")`; call `await route.action.instruction.run(createSession({ ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: storiesIndexPath, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.1" }))`; assert each case returns exact failed result `{ kind: "failed", errorMessage: "The selected story has an unsupported story status." }`.

    [x] Subtask 17.42: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts`, add selected-story derivation failure coverage for missing `story_file_name` that retrieves `const route = findRoute("step-1", "step-1-await-select-story-panel", "step-1-derive-selected-story-values")`, narrows with `expect(route.action.kind).to.equal("run_deterministic_procedure")` and `if (route.action.kind !== "run_deterministic_procedure") { throw new Error(\`Expected run_deterministic_procedure, received ${route.action.kind}.\`) }`, then uses a fixture created with `const fixtureRoot = await mkdtemp(join(tmpdir(), "pi-planning-workflow-"))`, `const implementationDir = join(fixtureRoot, "implementation")`, `await mkdir(implementationDir, { recursive: true })`, and `const storiesIndexPath = join(implementationDir, "epic-1-stories.index.json")` inside a `try` block whose `finally` block calls `await rm(fixtureRoot, { recursive: true, force: true })`; write a loose JSON story record with `await writeFile(storiesIndexPath, JSON.stringify({ version: 1, stories: [{ story_identity: "1.1", story_type: "primary", parent_story_identity: null, story_file_generated: true, status: "draft" }] }), "utf8")`; call `await route.action.instruction.run(createSession({ ...SAMPLE_WORKFLOW_VALUES, [PiPlanningWorkflowValueKey.StoriesIndex]: storiesIndexPath, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.1" }))`; assert exact failed result `{ kind: "failed", errorMessage: "I could not read or parse the selected story index before resolving the target story." }`.
    ...
    [x] Subtask 17.46: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningToolSchemas.test.ts`, import type `WorkflowPromptBuilderInput` and `WorkflowValues` from `../../../types`, import `PiPlanningEditIntent`, `PiPlanningWorkflowValueKey`, and `piPlanningWorkflowDefinition` from `../piPlanningWorkflow`, replace `PI_PLANNING_STEP_BUILDERS` with `PI_PLANNING_NON_STEP_6_BUILDERS` containing only Step 1 through Step 5 zero-argument builders, and add helper `createStep6ToolSchemaInput(editIntent: PiPlanningEditIntent | undefined): WorkflowPromptBuilderInput`. Inside that helper, declare `const workflowValues: WorkflowValues = { [PiPlanningWorkflowValueKey.ProjectTitle]: "PI Planning Project", [PiPlanningWorkflowValueKey.ProjectFolderName]: "pi-planning-project", [PiPlanningWorkflowValueKey.ArchitectureDocument]: "/tmp/pi-planning-project/planning/architecture.md", [PiPlanningWorkflowValueKey.EpicsDocument]: "/tmp/pi-planning-project/planning/Epics.md", [PiPlanningWorkflowValueKey.EpicsIndex]: "/tmp/pi-planning-project/planning/Epics.index.json", [PiPlanningWorkflowValueKey.BrainstormingDocument]: "/tmp/pi-planning-project/discovery/brainstorming.md", [PiPlanningWorkflowValueKey.AdditionalContext]: "/tmp/pi-planning-project/research/context.md", [PiPlanningWorkflowValueKey.TargetEpic]: "Epic 1: Improve workflow runtime", [PiPlanningWorkflowValueKey.EpicIdentity]: "1", [PiPlanningWorkflowValueKey.ImplementationFolder]: "/tmp/pi-planning-project/implementation", [PiPlanningWorkflowValueKey.DraftsFolder]: "/tmp/pi-planning-project/implementation/drafts", [PiPlanningWorkflowValueKey.StoriesIndex]: "/tmp/pi-planning-project/implementation/epic-1-stories.index.json", [PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart]: true, [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "1.1", [PiPlanningWorkflowValueKey.SelectedStoryFileName]: "Story-1-1.md", [PiPlanningWorkflowValueKey.SelectedStoryStatus]: "draft", [PiPlanningWorkflowValueKey.TargetStory]: "/tmp/pi-planning-project/implementation/drafts/Story-1-1.md" }`, then add exact guard `if (editIntent !== undefined) { workflowValues[PiPlanningWorkflowValueKey.EditIntent] = editIntent }`; return exact object fields `session`, `step: piPlanningWorkflowDefinition.steps["step-6"]`, and `renderWorkflowValue: (value): string => (typeof value === "string" ? value : JSON.stringify(value))`; the returned `session` must be a fully shaped `ActiveWorkflowSession` with `activeStepNumber: 6`, `workflowValues`, `projectSelection: { projectMode: "existing", projectTitle: "PI Planning Project", projectFolderName: "pi-planning-project" }`, `lifecycle: { projectSelectionCompleted: true }`, `entryArtifactResolution: undefined`, `ui: { formSession: undefined, stepResolutionSession: undefined, suppressedWorkflowFormIds: [], suppressedWorkflowStepResolutionRoutes: [] }`, and `branchContext: { activeBranchId: "step-6-project-prompt" }`.
...
[x] Task 18. Update prompt projection integration coverage.

Allowed files:
- `/Users/robertboston/Documents/Cline Extension/cline/src/core/prompts/system-prompt/__tests__/integration.test.ts`

    [x] Subtask 18.1: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/prompts/system-prompt/__tests__/integration.test.ts`, replace the existing `piPlanningWorkflowDefinition` import from `@/core/task/workflow-runtime/workflow-modules/pi-planning` with exact import `{ PiPlanningEditIntent, PiPlanningWorkflowValueKey, piPlanningWorkflowDefinition }` from `@/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow`, add constant `PI_PLANNING_TARGET_STORY = "/test/project/implementation/drafts/Story-7-1.md"`, and change `createPiPlanningWorkflowSession` to `const createPiPlanningWorkflowSession = (activeStepNumber: PiPlanningPromptStepNumber, options: { editIntent: PiPlanningEditIntent } | undefined = undefined): ActiveWorkflowSession => { ... }`. Inside the function, declare `const workflowValues: WorkflowValues = { [PiPlanningWorkflowValueKey.ArchitectureDocument]: PI_PLANNING_ARCHITECTURE_DOCUMENT, [PiPlanningWorkflowValueKey.EpicsDocument]: PI_PLANNING_EPICS_DOCUMENT, [PiPlanningWorkflowValueKey.EpicsIndex]: PI_PLANNING_EPICS_INDEX, [PiPlanningWorkflowValueKey.BrainstormingDocument]: PI_PLANNING_BRAINSTORMING_DOCUMENT, [PiPlanningWorkflowValueKey.AdditionalContext]: PI_PLANNING_ADDITIONAL_CONTEXT, [PiPlanningWorkflowValueKey.TargetEpic]: PI_PLANNING_TARGET_EPIC, [PiPlanningWorkflowValueKey.EpicIdentity]: PI_PLANNING_EPIC_IDENTITY, [PiPlanningWorkflowValueKey.ImplementationFolder]: PI_PLANNING_IMPLEMENTATION_FOLDER, [PiPlanningWorkflowValueKey.DraftsFolder]: PI_PLANNING_DRAFTS_FOLDER, [PiPlanningWorkflowValueKey.StoriesIndex]: PI_PLANNING_STORIES_INDEX, [PiPlanningWorkflowValueKey.StoriesIndexExistedAtWorkflowStart]: true, [PiPlanningWorkflowValueKey.ProjectTitle]: "PI Planning Session", [PiPlanningWorkflowValueKey.ProjectFolderName]: "pi-planning-session", [PiPlanningWorkflowValueKey.SelectedStoryIdentity]: "7.1", [PiPlanningWorkflowValueKey.SelectedStoryFileName]: "Story-7-1.md", [PiPlanningWorkflowValueKey.SelectedStoryStatus]: "draft", [PiPlanningWorkflowValueKey.TargetStory]: PI_PLANNING_TARGET_STORY }`; then add exact guard `if (options !== undefined) { workflowValues[PiPlanningWorkflowValueKey.EditIntent] = options.editIntent }`; return exact `ActiveWorkflowSession` object `{ activeStepNumber, workflowValues, projectSelection: { projectMode: "new", projectTitle: "PI Planning Session", projectFolderName: "pi-planning-session" }, lifecycle: { projectSelectionCompleted: true }, entryArtifactResolution: undefined, ui: { formSession: undefined, stepResolutionSession: undefined, suppressedWorkflowFormIds: [], suppressedWorkflowStepResolutionRoutes: [] }, branchContext: { activeBranchId: getPiPlanningEntryBranchId(activeStepNumber) } }`.

    [x] Subtask 18.2: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/prompts/system-prompt/__tests__/integration.test.ts`, update `buildPiPlanningPromptContext(activeStepNumber)` to accept second parameter `options: { editIntent: PiPlanningEditIntent } | undefined = undefined` and forward `options` to `createPiPlanningWorkflowSession(activeStepNumber, options)`.

    [x] Subtask 18.3: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/prompts/system-prompt/__tests__/integration.test.ts`, keep the active Step 6 initial-buildout projection test and call `buildPiPlanningPromptContext(6, { editIntent: PiPlanningEditIntent.CompleteInitialStoryBuildout })`; assert exact native tool names `list_files`, `read_file`, `apply_patch`, `plan_story_artifacts`, `generate_story_files`, `send_user_message`, `ask_followup_question`, and `attempt_completion`.

    [x] Subtask 18.4: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/prompts/system-prompt/__tests__/integration.test.ts`, add active Step 6 edit-existing-story projection coverage using `buildPiPlanningPromptContext(6, { editIntent: PiPlanningEditIntent.EditExistingStoryFile })`; assert exact native tool names `read_file`, `apply_patch`, `send_user_message`, `ask_followup_question`, and `attempt_completion`.

    [x] Subtask 18.5: In `/Users/robertboston/Documents/Cline Extension/cline/src/core/prompts/system-prompt/__tests__/integration.test.ts`, update pi-planning backend-only prompt projection coverage so the active step cases include Step 6 initial-buildout and Step 6 edit-existing-story projections, and assert `plan_story_artifacts` and `generate_story_files` are absent from the Step 6 edit-existing-story native tool list.

[x] Task 19. Run Phase 8 validation.

Allowed files:
- `/Users/robertboston/Documents/Cline Extension/cline/docs/workflows/workflow-runtime/workflow-modules/pi-planning/action-plan.md`

    [x] Subtask 19.1: Run `npm run test:unit -- src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningWorkflow.test.ts src/core/task/workflow-runtime/workflow-modules/pi-planning/__tests__/piPlanningToolSchemas.test.ts`.

    [x] Subtask 19.2: Run `npm run test:unit -- src/core/task/workflow-runtime/__tests__/WorkflowRuntime.test.ts src/core/task/tools/handlers/__tests__/PlanStoryArtifactsToolHandler.test.ts`.

    [x] Subtask 19.3: Run `npm run test:unit -- src/core/prompts/system-prompt/__tests__/integration.test.ts`.

    [x] Subtask 19.4: Run `npm run check-types`.

    [x] Subtask 19.5: Run `npm run lint`.

    [x] Subtask 19.6: Run `rg -n "step-1-required-context-panel|Required Context|Confirm the required context files" src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningWorkflow.ts src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningToolSchemas.ts src/core/task/workflow-runtime/workflow-modules/pi-planning/index.ts` and confirm no runtime source matches.

    [x] Subtask 19.7: Run `rg -n "resolve_story_index_target_story|WorkflowResolveStoryIndexTargetStory|WORKFLOW_STORY_STATUS_FOLDER_SEGMENTS" src/core/task/workflow-runtime src/core/task/workflow-runtime/workflow-modules/pi-planning` and confirm no source matches.

    [x] Subtask 19.8: Run `rg -n "build_workflow_document|create_workflow_artifact|archive_workflow_artifact|delete_workflow_artifact|move_workflow_project_file|execute_command|set_workflow_values" src/core/task/workflow-runtime/workflow-modules/pi-planning/piPlanningToolSchemas.ts` and confirm any `set_workflow_values` matches are limited to the Step 4 schema builder and the Step 4-only `buildPiPlanningSetWorkflowValuesToolSchema` helper, with no matches for the other guarded tool names.
