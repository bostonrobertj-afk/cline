...
/**
 * Handles converting any directory into a file.
 * The old .clinerules file will be renamed to a default filename
 ...
 * Create a rule file
 */
export const createRuleFile = async (isGlobal: boolean, filename: string, cwd: string, _type: string) => {
	try {
		let filePath: string
		if (isGlobal) {
