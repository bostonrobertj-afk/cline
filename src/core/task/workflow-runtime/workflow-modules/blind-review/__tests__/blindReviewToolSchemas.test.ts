...
describe("blindReviewToolSchemas", () => {
	it("returns an empty canonical tool-schema array for Step 1", () => {
		expect(buildBlindReviewStep1ToolSchemas()).to.deep.equal([])
	})

	it("exposes the exact Step 2 tool schema order", () => {
		expect(schemaNames(buildBlindReviewStep2ToolSchemas())).to.deep.equal(STEP_2_TOOL_NAMES)
	})

	it("uses the native GPT-5 schema variant and exact Step 2 parameter shapes", () => {
		const step2Schemas = buildBlindReviewStep2ToolSchemas()
	...
	it("does not expose forbidden model-facing tools in Step 1 or Step 2", () => {
		const stepToolNameSets: readonly (readonly string[])[] = [
			schemaNames(buildBlindReviewStep1ToolSchemas()),
			schemaNames(buildBlindReviewStep2ToolSchemas()),
		]
