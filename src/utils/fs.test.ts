...
describe("Filesystem Utilities", () => {
	...
	it("should exclude .clinerules/hooks directory specifically", async () => {
		...
		allFiles.length.should.equal(4) // 2 in root + 2 in hooks
		...
		filteredFiles.length.should.equal(2) // 2 in root

		const expectedFiles = [path.resolve(clinerulesDirPath, "config.json"), path.resolve(clinerulesDirPath, "settings.js")]
