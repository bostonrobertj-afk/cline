...
describe("slash-commands", () => {
	...
	describe("getMatchingSlashCommands with MCP servers", () => {
		...
		it("keeps backend-provided slash commands available without workflow-toggle inputs", () => {
			...
			const result = getMatchingSlashCommands("bmad", mcpServers, availableCommands)
	...
	describe("validateSlashCommand with MCP servers", () => {
		...
		it("validates backend-provided workflow commands", () => {
			const result = validateSlashCommand("bmad-code-review", [], [{ name: "bmad-code-review", section: "custom" }])
			expect(result).toBe("full")
		})
	})
